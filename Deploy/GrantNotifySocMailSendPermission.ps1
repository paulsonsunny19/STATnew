...
<#
.SYNOPSIS
    Grants the Microsoft Graph "Mail.Send" application permission to the NotifySocTeams
    Logic App's system-assigned managed identity.

.DESCRIPTION
    Run this ONCE, manually, after `LogicApp/NotifySocTeams/azuredeploy.json` has deployed the
    workflow, by an account holding Global Administrator or Privileged Role Administrator.
    ...
    permission only; it does NOT create that scoping policy - see the README in
    LogicApp/NotifySocTeams for the `New-ApplicationAccessPolicy` step, and do not skip it.

.PARAMETER LogicAppPrincipalId
    The NotifySocTeams workflow's managed identity Object (principal) ID - available as the
    `logicAppPrincipalId` output of LogicApp/NotifySocTeams/azuredeploy.json, or from:
...
Write-Host "tenant. Scope it down to only the SenderMailboxUpn used by NotifySocTeams with an" -ForegroundColor Cyan
Write-Host "Exchange Online application access policy - see LogicApp/NotifySocTeams/README.md." -ForegroundColor Cyan
